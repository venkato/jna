...
#include "com_sun_nik_win32_DLLCallback.h"
#ifdef _WIN64
#ifdef _MSC_VER
...
#define DLL_FPTRS com_sun_nik_win32_DLLCallback_DLL_FPTRS
void (*fn[DLL_FPTRS])();
...
#define HANDLER_TYPE "com/sun/nik/Callback$UncaughtExceptionHandler"
#define HANDLER_SIG "Lcom/sun/nik/Callback$UncaughtExceptionHandler;"
  jclass classHandler = (*env)->FindClass(env, HANDLER_TYPE);
  if (classHandler) {
    jclass classNative = (*env)->FindClass(env, "com/sun/nik/Native");
    if (classNative) {
      ...
      if (fid) {
        ...
        if (handler) {
          jmethodID mid = (*env)->GetMethodID(env, classHandler, "uncaughtException", "(Lcom/sun/nik/Callback;Ljava/lang/Throwable;)V");
          if (mid) {
