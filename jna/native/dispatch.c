...
#define MASK_CC          com_sun_nik_Function_MASK_CC
#define THROW_LAST_ERROR com_sun_nik_Function_THROW_LAST_ERROR
...
//1 nick begin nik
static jint nickDebugMode=0;
static jclass classFunctionExt;
static jclass classJniEnv;
static jmethodID simpleFunctionMethodId;
static jmethodID processParamsMethodId;
static jmethodID processResultBeforeExcMethodId;
static jmethodID processResultBeforeExc2MethodId;
//1 nick end nik
...
dispatch(JNIEnv *env, void* func, jint flags, jobjectArray arr,
         ffi_type *ffi_return_type, void *resP, jobject self)
{
  int i, nargs;
  jvalue* c_args;
  char array_pt;
  ...
  //nik b
    ...
    char functionExtFlag=1;

    if(nickDebugMode>0){
	  printf("nikjnadispatch: begin invoke\n");
	  printf("nikjnadispatch: function ref is null: %d\n", (self==NULL));
  }


  if(functionExtFlag==1){
	  if(nickDebugMode>0){
		  printf("nikjnadispatch: calling process params before\n");
	  }
	  (*env)->CallVoidMethod(env,self, processParamsMethodId, flags, arr, (unsigned long long)resP, (unsigned long long)env);
	  if(nickDebugMode>0){
		  printf("nikjnadispatch: call process params before finished\n");
	  }
  }
  //nik e
...
#define GET_ELEMS(TYPE) do {ptr=(*env)->Get##TYPE##ArrayElements(env,arg,NULL); release=(void*)(*env)->Release##TYPE##ArrayElements; }while(0)
    ...
    //7 nick begin
	  ...
	  if(nickDebugMode>0){
		  printf("nikjnadispatch: changing param to jni env, param order: %d\n",i);
	  }
		// printf("dispatch.c: arg is jni env\n");
	  c_args[i].l = env;
	  ffi_types[i] = &ffi_type_pointer;
	  ffi_values[i] = &c_args[i].l;
	}
  ...
  if(nickDebugMode>0){
	  printf("nikjnadispatch: just before call\n");
  }
  status = ffi_prep_cif(&cif, abi, nargs, ffi_return_type, ffi_types);
  if(nickDebugMode>0){
	  printf("nikjnadispatch: just after call\n");
  }
 ...
 cleanup:
  ...
  if (throw_type) {
	  if(nickDebugMode>0){
		  printf("nikjnadispatch: exception throw, no after result callback invoking\n");
	  }
    throwByName(env, throw_type, throw_msg);
  }else{
		if(functionExtFlag==1){
			  if(nickDebugMode>0){
				  printf("nikjnadispatch: just before result callback invoking\n");
			  }
	    	(*env)->CallVoidMethod(env,self, processResultBeforeExcMethodId, flags, arr,(unsigned long long)resP);
			  if(nickDebugMode>0){
					  printf("nikjnadispatch: just after result callback invoking\n");
			}
	    }
...
Java_com_sun_nik_Native_invokePointer(JNIEnv *env, jclass UNUSED(cls),
                                      jlong fp, jint callconv, jobjectArray arr,jobject functionExt)
{
    jvalue result;
    dispatch(env, L2A(fp), callconv, arr, &ffi_type_pointer, &result, functionExt);
    return A2L(result.l);
}
...
Java_com_sun_nik_Native_invokeObject(JNIEnv *env, jclass UNUSED(cls),
                                     jlong fp, jint callconv, jobjectArray arr,jobject functionExt)
{
    jvalue result;
    dispatch(env, L2A(fp), callconv, arr, &ffi_type_pointer, &result, functionExt);
 ...
 * Signature: (JI[Ljava/lang/Object;Lcom/sun/nik/Structure)LStructure;
 */
JNIEXPORT void JNICALL
Java_com_sun_nik_Native_invokeStructure(JNIEnv *env, jclass UNUSED(cls),
                                        jlong fp, jint callconv, jobjectArray arr,
                                        jlong memory, jlong type_info,jobject functionExt)
{
  ...
  if (!rtype) {
    ...
    dispatch(env, L2A(fp), callconv, arr, rtype, L2A(memory), functionExt);
  }
}
...
Java_com_sun_nik_Native_invokeDouble(JNIEnv *env, jclass UNUSED(cls),
                                     jlong fp, jint callconv, jobjectArray arr,jobject functionExt)
{
    jvalue result;
    dispatch(env, L2A(fp), callconv, arr, &ffi_type_double, &result, functionExt);
    return result.d;
}
...
Java_com_sun_nik_Native_invokeFloat(JNIEnv *env, jclass UNUSED(cls),
                                    jlong fp, jint callconv, jobjectArray arr,jobject functionExt)
{
    jvalue result;
    dispatch(env, L2A(fp), callconv, arr, &ffi_type_float, &result, functionExt);
    return result.f;
}
...
Java_com_sun_nik_Native_invokeInt(JNIEnv *env, jclass UNUSED(cls),
                                  jlong fp, jint callconv, jobjectArray arr,jobject functionExt)
{
    ffi_arg result;
    dispatch(env, L2A(fp), callconv, arr, &ffi_type_sint32, &result, functionExt);
    return (jint)result;
}
...
//nik b nick
JNIEXPORT jint JNICALL
Java_com_sun_nik_Native_invokeWithJniEnv(JNIEnv *env, jclass UNUSED(cls),
                                  jlong fp, jint callconv, jobjectArray arr,jobject functionExt)
{
    ffi_arg result;
    dispatch(env, L2A(fp), callconv, arr, &ffi_type_sint32, &result,functionExt);
...
JNIEXPORT void JNICALL
Java_com_sun_nik_Native_setNikDebugMode(JNIEnv *env, jclass UNUSED(classp), jint nickDebugModeNew) {
	printf("nikjnadispatch: set new debug mode = %d\n",nickDebugModeNew);
	nickDebugMode=nickDebugModeNew;
}

JNIEXPORT jint JNICALL
Java_com_sun_nik_Native_getNikDebugMode(JNIEnv *env, jclass UNUSED(classp)) {
  return nickDebugMode;
}
 ...
 */
JNIEXPORT jlong JNICALL
Java_com_sun_nik_Native_invokeLong(JNIEnv *env, jclass UNUSED(cls),
                                   jlong fp, jint callconv, jobjectArray arr,jobject functionExt)
{
    jvalue result;
    dispatch(env, L2A(fp), callconv, arr, &ffi_type_sint64, &result, functionExt);
    return result.j;
}
 ...
 */
JNIEXPORT void JNICALL
Java_com_sun_nik_Native_invokeVoid(JNIEnv *env, jclass UNUSED(cls),
                                   jlong fp, jint callconv, jobjectArray arr,jobject functionExt)
{
    jvalue result;
    dispatch(env, L2A(fp), callconv, arr, &ffi_type_void, &result, functionExt);
}

JNIEXPORT jlong JNICALL
Java_com_sun_nik_Native_createNativeCallback(JNIEnv *env,
                                             jclass UNUSED(cls),
                                             jobject obj,
                                             jobject method,
...
JNIEXPORT void JNICALL
Java_com_sun_nik_Native_freeNativeCallback(JNIEnv *env,
                                           jclass UNUSED(cls),
                                           jlong ptr) {
 ...
 */
JNIEXPORT jlong JNICALL
Java_com_sun_nik_Native_open(JNIEnv *env, jclass UNUSED(cls), jstring lib, jint flags){
 ...
 */
JNIEXPORT void JNICALL
Java_com_sun_nik_Native_close(JNIEnv *env, jclass UNUSED(cls), jlong handle)
{
 ...
 */
JNIEXPORT jlong JNICALL
Java_com_sun_nik_Native_findSymbol(JNIEnv *env, jclass UNUSED(cls),
 ...
 * Signature: (J[BII)V
 */
JNIEXPORT void JNICALL Java_com_sun_nik_Native_write__J_3BII
 ...
 * Signature: (J[CII)V
 */
JNIEXPORT void JNICALL Java_com_sun_nik_Native_write__J_3CII
 ...
 * Signature: (J[DII)V
 */
JNIEXPORT void JNICALL Java_com_sun_nik_Native_write__J_3DII
 ...
 * Signature: (J[FII)V
 */
JNIEXPORT void JNICALL Java_com_sun_nik_Native_write__J_3FII
 ...
 * Signature: (J[III)V
 */
JNIEXPORT void JNICALL Java_com_sun_nik_Native_write__J_3III
 ...
 * Signature: (J[JII)V
 */
JNIEXPORT void JNICALL Java_com_sun_nik_Native_write__J_3JII
 ...
 * Signature: (J[SII)V
 */
JNIEXPORT void JNICALL Java_com_sun_nik_Native_write__J_3SII
 ...
 * Signature: (JB)J
 */
JNIEXPORT jlong JNICALL Java_com_sun_nik_Native_indexOf__JB
 ...
 * Signature: (J[BII)V
 */
JNIEXPORT void JNICALL Java_com_sun_nik_Native_read__J_3BII
(JNIEnv *env, jclass UNUSED(cls), jlong addr, jbyteArray arr, jint off, jint n)
{
  PSTART();
 ...
 * Signature: (J[CII)V
 */
JNIEXPORT void JNICALL Java_com_sun_nik_Native_read__J_3CII
    (JNIEnv *env, jclass UNUSED(cls), jlong addr, jcharArray arr, jint off, jint n)
{
 ...
 * Signature: (J[DII)V
 */
JNIEXPORT void JNICALL Java_com_sun_nik_Native_read__J_3DII
    (JNIEnv *env, jclass UNUSED(cls), jlong addr, jdoubleArray arr, jint off, jint n)
{
  PSTART();
 ...
 * Signature: (J[FII)V
 */
JNIEXPORT void JNICALL Java_com_sun_nik_Native_read__J_3FII
    (JNIEnv *env, jclass UNUSED(cls), jlong addr, jfloatArray arr, jint off, jint n)
{
  PSTART();
 ...
 * Signature: (J[III)V
 */
JNIEXPORT void JNICALL Java_com_sun_nik_Native_read__J_3III
    (JNIEnv *env, jclass UNUSED(cls), jlong addr, jintArray arr, jint off, jint n)
{
  PSTART();
 ...
 * Signature: (J[JII)V
 */
JNIEXPORT void JNICALL Java_com_sun_nik_Native_read__J_3JII
    (JNIEnv *env, jclass UNUSED(cls), jlong addr, jlongArray arr, jint off, jint n)
{
  PSTART();
 ...
 * Signature: (J[SII)V
 */
JNIEXPORT void JNICALL Java_com_sun_nik_Native_read__J_3SII
    (JNIEnv *env, jclass UNUSED(cls), jlong addr, jshortArray arr, jint off, jint n)
{
  PSTART();
 ...
 * Signature: (J)B
 */
JNIEXPORT jbyte JNICALL Java_com_sun_nik_Native_getByte
 ...
 * Signature: (J)C
 */
JNIEXPORT jchar JNICALL Java_com_sun_nik_Native_getChar
(JNIEnv * UNUSED_ENV(env), jclass UNUSED(cls), jlong addr)
{
    wchar_t res = 0;
    MEMCPY(env, &res, L2A(addr), sizeof(res));
 ...
 * Class:     Native
 * Method:    _getPointer
 * Signature: (J)Lcom/sun/nik/Pointer;
 */
JNIEXPORT jlong JNICALL Java_com_sun_nik_Native__1getPointer
(JNIEnv *UNUSED_ENV(env), jclass UNUSED(cls), jlong addr)
{
    void *ptr = NULL;
...
/*
 * Class:     com_sun_nik_Native
 * Method:    _getDirectByteBuffer
 * Signature: (JJ)Ljava/nio/ByteBuffer;
 */
JNIEXPORT jobject JNICALL Java_com_sun_nik_Native_getDirectByteBuffer
 ...
 * Signature: (J)D
 */
JNIEXPORT jdouble JNICALL Java_com_sun_nik_Native_getDouble
(JNIEnv * UNUSED_ENV(env), jclass UNUSED(cls), jlong addr)
{
    jdouble res = 0;
    MEMCPY(env, &res, L2A(addr), sizeof(res));
 ...
 * Signature: (J)F
 */
JNIEXPORT jfloat JNICALL Java_com_sun_nik_Native_getFloat
(JNIEnv * UNUSED_ENV(env), jclass UNUSED(cls), jlong addr)
{
    jfloat res = 0;
    MEMCPY(env, &res, L2A(addr), sizeof(res));
 ...
 * Signature: (J)I
 */
JNIEXPORT jint JNICALL Java_com_sun_nik_Native_getInt
(JNIEnv * UNUSED_ENV(env), jclass UNUSED(cls), jlong addr)
{
    jint res = 0;
    MEMCPY(env, &res, L2A(addr), sizeof(res));
 ...
 * Signature: (J)J
 */
JNIEXPORT jlong JNICALL Java_com_sun_nik_Native_getLong
(JNIEnv * UNUSED_ENV(env), jclass UNUSED(cls), jlong addr)
{
    jlong res = 0;
    MEMCPY(env, &res, L2A(addr), sizeof(res));
 ...
 * Signature: (J)S
 */
JNIEXPORT jshort JNICALL Java_com_sun_nik_Native_getShort
(JNIEnv * UNUSED_ENV(env), jclass UNUSED(cls), jlong addr)
{
    jshort res = 0;
    MEMCPY(env, &res, L2A(addr), sizeof(res));
 ...
 */
JNIEXPORT jstring JNICALL Java_com_sun_nik_Native_getWideString
 ...
 * Signature: (JB)LB;
 */
JNIEXPORT jbyteArray JNICALL Java_com_sun_nik_Native_getStringBytes
(JNIEnv *env, jclass UNUSED(cls), jlong addr)
{
 ...
 * Signature: (JJB)V
 */
JNIEXPORT void JNICALL Java_com_sun_nik_Native_setMemory
 ...
 * Signature: (JB)V
 */
JNIEXPORT void JNICALL Java_com_sun_nik_Native_setByte
(JNIEnv * UNUSED_ENV(env), jclass UNUSED(cls), jlong addr, jbyte value)
{
 ...
 * Signature: (JC)V
 */
JNIEXPORT void JNICALL Java_com_sun_nik_Native_setChar
 ...
 * Signature: (JJ)V
 */
JNIEXPORT void JNICALL Java_com_sun_nik_Native_setPointer
 ...
 * Signature: (JD)V
 */
JNIEXPORT void JNICALL Java_com_sun_nik_Native_setDouble
 ...
 * Signature: (JF)V
 */
JNIEXPORT void JNICALL Java_com_sun_nik_Native_setFloat
 ...
 * Signature: (JI)V
 */
JNIEXPORT void JNICALL Java_com_sun_nik_Native_setInt
 ...
 * Signature: (JJ)V
 */
JNIEXPORT void JNICALL Java_com_sun_nik_Native_setLong
(JNIEnv * UNUSED_ENV(env), jclass UNUSED(cls), jlong addr, jlong value)
{
  MEMCPY(env, L2A(addr), &value, sizeof(value));
}
 ...
 * Signature: (JS)V
 */
JNIEXPORT void JNICALL Java_com_sun_nik_Native_setShort
 ...
 */
JNIEXPORT void JNICALL Java_com_sun_nik_Native_setWideString
 ...
 * Signature: (J)J
 */
JNIEXPORT jlong JNICALL Java_com_sun_nik_Native_malloc
 ...
 * Signature: (J)V
 */
JNIEXPORT void JNICALL Java_com_sun_nik_Native_free
 ...
 */
JNIEXPORT jint JNICALL
Java_com_sun_nik_Native_sizeof(JNIEnv *env, jclass UNUSED(cls), jint type)
{
  switch(type) {
  case com_sun_nik_Native_TYPE_VOIDP: return sizeof(void*);
  case com_sun_nik_Native_TYPE_LONG: return sizeof(long);
  case com_sun_nik_Native_TYPE_WCHAR_T: return sizeof(wchar_t);
  case com_sun_nik_Native_TYPE_SIZE_T: return sizeof(size_t);
  default:
    {
      char msg[MSG_SIZE];
 ...
 */
JNIEXPORT void JNICALL
Java_com_sun_nik_Native_initIDs(JNIEnv *env, jclass cls) {
  if (!LOAD_CREF(env, Pointer, "com/sun/nik/Pointer")) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain class com.sun.nik.Pointer");
  }
  else if (!LOAD_MID(env, MID_Pointer_init, classPointer,
                     "<init>", "(J)V")) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain constructor for class com.sun.nik.Pointer");
  }
  else if (!LOAD_FID(env, FID_Pointer_peer, classPointer, "peer", "J")) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain peer field ID for class com.sun.nik.Pointer");
  }
  else if (!(classNative = (*env)->NewWeakGlobalRef(env, cls))) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain global reference for class com.sun.nik.Native");
  }
  else if (!(MID_Native_dispose
             = (*env)->GetStaticMethodID(env, classNative,
                                         "dispose", "()V"))) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain static method dispose from class com.sun.nik.Native");
  }
  else if (!(MID_Native_fromNative
             = (*env)->GetStaticMethodID(env, classNative,
                                         "fromNative", "(Ljava/lang/Class;Ljava/lang/Object;)Lcom/sun/nik/NativeMapped;"))) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain static method fromNative from class com.sun.nik.Native");
  }
  else if (!(MID_Native_nativeType
             = (*env)->GetStaticMethodID(env, classNative,
                                         "nativeType", "(Ljava/lang/Class;)Ljava/lang/Class;"))) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain static method nativeType from class com.sun.nik.Native");
  }
  else if (!(MID_Native_toNativeTypeMapped
             = (*env)->GetStaticMethodID(env, classNative,
                                         "toNative", "(Lcom/sun/nik/ToNativeConverter;Ljava/lang/Object;)Ljava/lang/Object;"))) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain static method toNative from class com.sun.nik.Native");
  }
  else if (!(MID_Native_fromNativeTypeMapped
             = (*env)->GetStaticMethodID(env, classNative,
                                         "fromNative", "(Lcom/sun/nik/FromNativeConverter;Ljava/lang/Object;Ljava/lang/Class;)Ljava/lang/Object;"))) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain static method fromNative from class com.sun.nik.Native");
  }
  else if (!LOAD_CREF(env, Structure, "com/sun/nik/Structure")) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain class com.sun.nik.Structure");
  }
  else if (!LOAD_MID(env, MID_Structure_getTypeInfo, classStructure,
                     "getTypeInfo", "()Lcom/sun/nik/Pointer;")) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain getTypeInfo method for class com.sun.nik.Structure");
  }
  else if (!(MID_Structure_newInstance
             = (*env)->GetStaticMethodID(env, classStructure,
                                         "newInstance", "(Ljava/lang/Class;J)Lcom/sun/nik/Structure;"))) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain static newInstance method for class com.sun.nik.Structure");
  }
  else if (!LOAD_MID(env, MID_Structure_read, classStructure,
                     "autoRead", "()V")) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain read method for class com.sun.nik.Structure");
  }
  else if (!LOAD_MID(env, MID_Structure_write, classStructure,
                     "autoWrite", "()V")) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain write method for class com.sun.nik.Structure");
  }
  else if (!LOAD_FID(env, FID_Structure_memory, classStructure, "memory", "Lcom/sun/nik/Pointer;")) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain memory field ID for class com.sun.nik.Structure");
  }
  else if (!LOAD_FID(env, FID_Structure_typeInfo, classStructure, "typeInfo", "J")) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain typeInfo field ID for class com.sun.nik.Structure");
  }
  else if (!LOAD_CREF(env, StructureByValue, "com/sun/nik/Structure$ByValue")) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain class com.sun.nik.Structure.ByValue");
  }
  else if (!LOAD_CREF(env, Callback, "com/sun/nik/Callback")) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain class com.sun.nik.Callback");
  }
  else if (!LOAD_CREF(env, AttachOptions, "com/sun/nik/CallbackReference$AttachOptions")) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain class com.sun.nik.CallbackReference.AttachOptions");
  }
  else if (!LOAD_CREF(env, CallbackReference, "com/sun/nik/CallbackReference")) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain class com.sun.nik.CallbackReference");
  }
  else if (!(MID_CallbackReference_getCallback
             = (*env)->GetStaticMethodID(env, classCallbackReference,
                                         "getCallback", "(Ljava/lang/Class;Lcom/sun/nik/Pointer;Z)Lcom/sun/nik/Callback;"))) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain static method getCallback from class com.sun.nik.CallbackReference");
  }
  else if (!(MID_CallbackReference_getFunctionPointer
             = (*env)->GetStaticMethodID(env, classCallbackReference,
                                         "getFunctionPointer", "(Lcom/sun/nik/Callback;Z)Lcom/sun/nik/Pointer;"))) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain static method getFunctionPointer from class com.sun.nik.CallbackReference");
  }
  else if (!(MID_CallbackReference_getNativeString
             = (*env)->GetStaticMethodID(env, classCallbackReference,
                                         "getNativeString", "(Ljava/lang/Object;Z)Lcom/sun/nik/Pointer;"))) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain static method getNativeString from class com.sun.nik.CallbackReference");
  }
  else if (!(MID_CallbackReference_initializeThread
             = (*env)->GetStaticMethodID(env, classCallbackReference,
                                         "initializeThread", "(Lcom/sun/nik/Callback;Lcom/sun/nik/CallbackReference$AttachOptions;)Ljava/lang/ThreadGroup;"))) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain static method initializeThread from class com.sun.nik.CallbackReference");
  }
  else if (!LOAD_CREF(env, WString, "com/sun/nik/WString")) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain class com.sun.nik.WString");
  }
  else if (!LOAD_CREF(env, NativeMapped, "com/sun/nik/NativeMapped")) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain class com.sun.nik.NativeMapped");
  }
  else if (!LOAD_MID(env, MID_NativeMapped_toNative, classNativeMapped,
                     "toNative", "()Ljava/lang/Object;")) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain toNative method for class com.sun.nik.NativeMapped");
  }
  else if (!LOAD_CREF(env, IntegerType, "com/sun/nik/IntegerType")) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain class com.sun.nik.IntegerType");
  }
  else if (!LOAD_FID(env, FID_IntegerType_value, classIntegerType, "value", "J")) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain value field ID for class com.sun.nik.IntegerType");
  }
  else if (!LOAD_CREF(env, PointerType, "com/sun/nik/PointerType")) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain class com.sun.nik.PointerType");
  }
  else if (!LOAD_FID(env, FID_PointerType_pointer, classPointerType, "pointer", "Lcom/sun/nik/Pointer;")) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain typeInfo field ID for class com.sun.nik.Structure");
  }
    ...
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain constructor for class com.sun.nik.WString");
  }
  else if (!LOAD_CREF(env, _ffi_callback, "com/sun/nik/Native$ffi_callback")) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain class com.sun.nik.Native$ffi_callback");
  }
  else if (!LOAD_MID(env, MID_ffi_callback_invoke, class_ffi_callback,
                     "invoke", "(JJJ)V")) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain invoke method from class com.sun.nik.Native$ffi_callback");
  }
  //2 nick begin nik
  else if (!LOAD_CREF(env, FunctionExt, "com/sun/nik/FunctionExt")) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain class com.sun.nik.FunctionExt");
  }else if (!LOAD_CREF(env, JniEnv, "com/sun/nik/JniEnvIdentifier")) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain class com.sun.nik.JniEnvIdentifier");
  }
  /*
  jmethodID processParamsMethodId = env->GetMethodID(classObjectWrapper, "processParams", "(ILjava/lang/Object;I)V");
  jmethodID processResultBeforeExcMethodId = env->GetMethodID(classObjectWrapper, "processResultBeforeExc", "(ILjava/lang/Object;I)V");
  jmethodID processResultBeforeExc2MethodId = env->GetMethodID(classObjectWrapper, "processResultBeforeExc2", "(ILjava/lang/Object;I)V");
  else if (!LOAD_CREF(env, ObjectWrapper, "org/isty/viewer/ObjectWrapper")) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain class org.isty.viewer.ObjectWrapper");
  }
  else if (!LOAD_FID(env, classObjectWrapperField, classObjectWrapper, "object", "Ljava/lang/Object;")) {
    throwByName(env, EUnsatisfiedLink,
                "Nick: Can't obtain object field for class classObjectWrapper");
  }
  */
  else if (!LOAD_MID(env, simpleFunctionMethodId, classFunctionExt,
                     "simpleFunction", "()V")) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain nick2 simpleFunction");
  }
  else if (!LOAD_MID(env, processParamsMethodId, classFunctionExt,
                     "processParams", "(I[Ljava/lang/Object;JJ)V")) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain nick3 processParams");
  }
  else if (!LOAD_MID(env, processResultBeforeExcMethodId, classFunctionExt,
                     "processResult", "(I[Ljava/lang/Object;J)V")) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain nick processResult");
  }
  //nick end nik

  // Initialize type fields within Structure.FFIType
  else {
#define CFFITYPE "com/sun/nik/Structure$FFIType$FFITypes"
    ...
    for (i=0;i < sizeof(fields)/sizeof(fields[0]);i++) {
      ...
      fid = (*env)->GetStaticFieldID(env, cls, field, "Lcom/sun/nik/Pointer;");
      if (!fid) {
...
JNIEXPORT jlong JNICALL
Java_com_sun_nik_Native_getWindowHandle0(JNIEnv* UNUSED_JAWT(env), jclass UNUSED(classp), jobject UNUSED_JAWT(w)) {
  return 0;
}

JNIEXPORT jlong JNICALL
Java_com_sun_nik_Native__1getDirectBufferPointer(JNIEnv *env, jclass UNUSED(classp), jobject buffer) {
  void* addr = NULL;
#ifndef NO_NIO_BUFFERS
...
JNIEXPORT void JNICALL
Java_com_sun_nik_Native_setProtected(JNIEnv *UNUSED(env), jclass UNUSED(classp), jboolean protect_access) {
#ifdef HAVE_PROTECTION
...
Java_com_sun_nik_Native_isProtected(JNIEnv *UNUSED(env), jclass UNUSED(classp)) {
  return is_protected();
}

JNIEXPORT void JNICALL
Java_com_sun_nik_Native_setLastError(JNIEnv *env, jclass UNUSED(classp), jint code) {
  JNA_set_last_error(env, code);
  SET_LAST_ERROR(code);
}

JNIEXPORT jint JNICALL
Java_com_sun_nik_Native_getLastError(JNIEnv *env, jclass UNUSED(classp)) {
...
Java_com_sun_nik_Native_getNativeVersion(JNIEnv *env, jclass UNUSED(classp)) {
...
JNIEXPORT jstring JNICALL
Java_com_sun_nik_Native_getAPIChecksum(JNIEnv *env, jclass UNUSED(classp)) {
#ifndef CHECKSUM
...
JNIEXPORT void JNICALL
Java_com_sun_nik_Native_unregister(JNIEnv *env, jclass UNUSED(ncls), jclass cls, jlongArray handles) {
...
JNIEXPORT jlong JNICALL
Java_com_sun_nik_Native_registerMethod(JNIEnv *env, jclass UNUSED(ncls),
...
JNIEXPORT void JNICALL
Java_com_sun_nik_Native_ffi_1call(JNIEnv *UNUSED(env), jclass UNUSED(cls), jlong cif, jlong fptr, jlong resp, jlong args)
{
  ffi_call(L2A(cif), FFI_FN(L2A(fptr)), L2A(resp), L2A(args));
}

JNIEXPORT jlong JNICALL
Java_com_sun_nik_Native_ffi_1prep_1cif(JNIEnv *env, jclass UNUSED(cls), jint abi, jint nargs, jlong ffi_return_type, jlong ffi_types)
{
...
JNIEXPORT jlong JNICALL
Java_com_sun_nik_Native_ffi_1prep_1closure(JNIEnv *env, jclass UNUSED(cls), jlong cif, jobject obj)
{
...
JNIEXPORT void JNICALL
Java_com_sun_nik_Native_ffi_1free_1closure(JNIEnv *env, jclass UNUSED(cls), jlong closure) {
...
JNIEXPORT jint JNICALL
Java_com_sun_nik_Native_initialize_1ffi_1type(JNIEnv *env, jclass UNUSED(cls), jlong type_info) {
...
JNIEXPORT void JNICALL
Java_com_sun_nik_Native_setDetachState(JNIEnv* env, jclass UNUSED(cls), jboolean d, jlong flag) {
