...
//1 nick end
//static jclass classObjectWrapper;
//static jfieldID classObjectWrapperField;
         ...
         ffi_type *ffi_return_type, void *resP)
{
  ...
  void** ffi_values;
  //void* funcResultNick;
  //jobject funcResultObjectWrapperNick=NULL;
  ffi_abi abi;
  ffi_status status;
  char msg[128];
  ...
  //funcResultNick=NULL;
	//3 nick begin
	char functionExtFlag=0;
	if ((*env)->IsInstanceOf(env, self, classFunctionExt)) {
		 //printf("instance of nick 87\n");
		functionExtFlag=1;
	}

	 if(functionExtFlag==1){
		 //printf("call simple func begin 3\n");
	    //(*env)->CallVoidMethod(env,self, simpleFunctionMethodId);
		 //printf("call simple func end 3\n");
		 //printf("call processParamsMethodId begin 3\n");
	    (*env)->CallVoidMethod(env,self, processParamsMethodId, flags, arr, (unsigned long long)resP, (unsigned long long)env);
		 //printf("call processParamsMethodId end\n");
	 }
	//void processResultBeforeExc(int flags, Object[] params,int resultPointer)
	//void processResultBeforeExc2(int flags, Object[] params,int resultPointer)
	//  processParams(int flags, Object[] params,int resultPointer);
...
#define GET_ELEMS(TYPE) do {ptr=(*env)->Get##TYPE##ArrayElements(env,arg,NULL); release=(void*)(*env)->Release##TYPE##ArrayElements; }while(0)
    ...
    /*
	 else if ((*env)->IsInstanceOf(env, arg, classObjectWrapper)) {
       printf("nick mappring");
       funcResultObjectWrapperNick=arg;
       funcResultNick=malloc(4);
       c_args[i].l = funcResultNick;
       ffi_types[i] = &ffi_type_pointer;
       ffi_values[i] = &c_args[i].l;
	}
	*/
  ...
  if (!ffi_error(env, "Native call setup", status)) {
    ...
    //6 nick begin
    if(functionExtFlag==1){
    	//printf("call processResultBeforeExcMethodId\n");
  ...
  //env->CallVoidMethod(self, processResultBeforeExc2MethodId, flags, arr,(int)resP);
  /*
  if(funcResultObjectWrapperNick!=NULL){
	  printf("process nik res\n");
	  jobject* valldd=(jobject*)funcResultNick;
	  (*env)->SetObjectField(env,funcResultObjectWrapperNickclassObjectWrapper,Field,*valldd);
	  printf("process nik res end\n");
  }
  */
 cleanup:
...
Java_com_sun_jna_Native_initIDs(JNIEnv *env, jclass cls) {
  ...
  //2 nick begin
  else if (!LOAD_CREF(env, FunctionExt, "com/sun/jna/FunctionExt")) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain class com.sun.jna.FunctionExt");
  }else if (!LOAD_CREF(env, JniEnv, "org/jna/jvmtiutils/JniEnvIdentifier")) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain class org.jna.jvmtiutils.JniEnvIdentifier");
  }
  /*
  ...
  */
    ...
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain nick3 processParams");
                     ...
                     "processResultBeforeExc", "(I[Ljava/lang/Object;J)V")) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain nick processResultBeforeExc");
  }
  else if (!LOAD_MID(env, processResultBeforeExc2MethodId, classFunctionExt,
                     "processResultBeforeExc2", "(I[Ljava/lang/Object;J)V")) {
    throwByName(env, EUnsatisfiedLink,
                "Can't obtain nick processResultBeforeExc2");
  }
  //nick end
